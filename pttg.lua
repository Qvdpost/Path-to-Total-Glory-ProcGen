...
function generate_room_type(room_chances, available_room_count)
    local acc = {}
...
function rule_assignable_to_row(n, room)
...
function filter_redundant_edges(map)
...
function create_paths(nodes, path_density)
...
function _create_paths(nodes, edge)
    local min
    local max
...
function get_common_ancestor(map, node1, node2, max_depth)
...
function create_nodes(height, width)
    local nodes_y = {}
    for y = 1, height do
        local nodes_x = {}
        for x = 1, width do
...
function count_connected_nodes(map)
    local count = 0
...
function distribute_rooms_across_map(map, room_list)
...
function last_minute_node_checker(map)
    for _, row in ipairs(map) do
        for _, node in ipairs(row) do
            if #node.edges > 0 and node.class == nil then
...
function get_next_room_type(map, n, room_list)
...
function rule_parent_matches(map, parents, room)
...
function rule_sibling_matches(sibs, room)
    local applicable_rooms = {
        [RoomType.RestRoom] = true,
        [RoomType.TreasureRoom] = true,
        [RoomType.ShopRoom] = true,
...
function get_siblings(map, node)
    local siblings = {}
...
function assign_rooms_to_nodes(map, room_list)
    local height = #map
    local width = #map[1]
...
function generate_maps(seed, map_height, map_width, path_density, ascension)
...
function main()
    ...
    -- local seed = 34
    local seed = os.clock() * 1000000

    local maps = generate_maps(seed, map_height, map_width, path_density, 1)

    for i, map in ipairs(maps) do
        print(string.format("\nAct %d", i))
        print(format_map(map, nil))

        print("Where do you wish to start?")
        local prompt = io.read("*n")
        local cursor = map[1][prompt]

        if not cursor.class then
            print("Dieded")
            return 0
        end
        ...
        while true do
            ...
            if can_move(map, cursor, prompt) then
                cursor = can_move(map, cursor, prompt)
            else
                print("Impossible move.")
            end

            if not cursor or not cursor.class then
                print("Dieded")
                return 0
            end
        end
...
function can_move(map, cursor, direction)
    if direction == 'l' and cursor.x > 1 then
        for _, edge in ipairs(cursor.edges) do
            if edge.dst_y == cursor.y + 1 and edge.dst_x == cursor.x - 1 then
                return map[cursor.y + 1][cursor.x - 1]
            end
        end
    elseif direction == 'm' then
        for _, edge in ipairs(cursor.edges) do
            if edge.dst_y == cursor.y + 1 and edge.dst_x == cursor.x then
                return map[cursor.y + 1][cursor.x]
            end
        end
